...
const candidateArchive = resolve(process.env.CALENDAR_UPGRADE_ARCHIVE ?? "apps/calendar/calendar.v0.6.8.neutron");

test("confirmed and live-hold Rendezvous calendar state survives Calendar 0.2.0 to 0.6.8", async ({ browser }) => {
  ...
  try {
    ...
    expect(Number(afterApp?.version)).toBe(608);
    ...
    // is left with Calendar 0.6.8 on both sides for the full Rendezvous suite.
    ...
    expect(Number(aliceAfterApp?.version)).toBe(608);
