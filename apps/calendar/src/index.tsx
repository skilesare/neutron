...
export const App = () => {
  ...
  const saveEvent = async () => {
    ...
    try {
      ...
      const problem = resultError(result); if (problem) throw new Error(problem); setDraft(freshDraft(undefined, undefined, false, activeZone)); await refresh();
    } catch (error) { setMessage(errorText(error)); } finally { setBusy(false); }
  };
  const removeEvent = async () => {
    if (!deleteArmed) { setDeleteArmed(true); return; }
    if (!draft.seriesId) return; setBusy(true);
    try { const result = draft.editScope === "occurrence" && draft.occurrenceId ? await updateSelf<JsonValue>("calendar_occurrence_remove_v2", [{ occurrence_id: draft.occurrenceId, expected_occurrence_revision: draft.occurrenceRevision }]) : await updateSelf<JsonValue>("calendar_series_remove_v2", [{ series_id: draft.seriesId, expected_series_revision: draft.seriesRevision }]); const problem = resultError(result); if (problem) throw new Error(problem); setDeleteArmed(false); setDraft(freshDraft(undefined, undefined, false, activeZone)); await refresh(); } catch (error) { setMessage(errorText(error)); } finally { setBusy(false); }
  };
