...
// Models sometimes emit small decimal identifiers as JSON numbers even when the
// descriptor requests strings. Accept only exactly representable integers on
// input, then normalize them to decimal strings before crossing the canister
// boundary. Outputs remain strings so Nat64 values can never lose precision.
const naturalInputSchema: JsonObject = { oneOf: [decimalSchema, { type: "integer", minimum: 0, maximum: Number.MAX_SAFE_INTEGER }] };
...
registerTool("search_events", {
  ...
  inputSchema: objectSchema([], { query: { type: "string", maxLength: 256 }, start: nullableRfc3339Schema, end: nullableRfc3339Schema, source: { type: ["string", "null"], enum: ["owner", "rendezvous", null] }, availability: { type: ["string", "null"], enum: ["busy", "free", null] }, recurring: { type: ["boolean", "null"] }, expectedRevision: { oneOf: [naturalInputSchema, { type: "null" }] }, offset: naturalInputSchema, limit: { type: "integer", minimum: 1, maximum: 100 } }),
...
registerTool("list_schedule", {
  ...
  inputSchema: objectSchema(["start", "end"], { start: rfc3339Schema, end: rfc3339Schema, offset: naturalInputSchema, limit: { type: "integer", minimum: 1, maximum: 100 } }), outputSchema: objectSchema(["revision", "total", "events"], { revision: decimalSchema, total: decimalSchema, events: { type: "array", items: eventSchema, maxItems: 100 } }), annotations: { "neutron:effects": ["read"] },
}, async (args, context) => {
...
function mutationOk(value: JsonValue, label: string): JsonObject { const result = record(value, label); if ("err" in result) { const error = record(result.err, "Calendar error"); throw new Error(`${text(error.code, "error code")}: ${text(error.message, "error message")} (Calendar revision ${nat(error.revision, "revision")})`); } return "ok" in result ? record(result.ok, `${label} result`) : result; }
