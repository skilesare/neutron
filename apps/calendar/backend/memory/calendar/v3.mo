// Persistent schema: keep immutable after Calendar 0.4.0 is released.
module {
    ...
    // Import identity is namespaced so unrelated producers may reuse a UID.
    // The digest covers the normalized supported VEVENT subset, not raw input.
    public type ImportProvenance = {
        series_id : Nat64;
    ...
    public type Reminder = {
        series_id : Nat64;
        occurrence_id : ?Nat64;
        offset_minutes : Nat32;
    };

    public type SeriesSnapshot = {
        series : EventSeries;
    ...
    public type BulkUndoChange = {
        #created : { series_id : Nat64 };
        #replaced : SeriesSnapshot;
        #deleted : SeriesSnapshot;
    };

    public type BulkReceipt = {
        batch_id : Blob;
        preview_digest : Blob;
        committed_revision : Nat64;
        created_at_ns : Nat64;
        changes : [BulkUndoChange];
        undone_at_revision : ?Nat64;
    };
